...
def updateColumns(columns, valToCk):
    ...
    for column in columns:
        ...
        for (station_number) in db.cursor:
            avgOfTmax = db.query(("SELECT avg(" + columnName + "),station_number from weather where station_number=%s"),
                                 (station_number))
            for x in avgOfTmax:
                db.insert((
                                      "UPDATE weather SET " + columnName + "=%s WHERE station_number=%s and TRIM(" + columnName + ")=%s"),
                          (x['avg(' + columnName + ')'], x['station_number'], valToCk))
...
def updateColumnsByMinValue(columns, valToCk):
    ...
    for column in columns:
        columnName = column
        x = db.query((
                                 "SELECT distinct " + columnName + " FROM weather where trim(" + columnName + ")!=%s ORDER BY " + columnName + " ASC"),
                     (valToCk))
        for minValue in x:
            if (float(minValue[columnName]) > 0):
                db.insert(("UPDATE weather SET " + columnName + "=%s WHERE TRIM(" + columnName + ")=%s"),
                          (minValue[columnName], valToCk))
                break;
    db.connection.commit()
    db.cursor.close()
